...
class Hub(GenericStrategy):
    ...
    def apply_strategy(self, network, param_value):
        ...
        hubs = list(network.get_graph().degree())
        hubs.sort( reverse=True, key= lambda x: x[1] )
        hubs = [ node for (node, degree) in hubs[ 0 : n_states ] ]

        for node in hubs:
            network.set_node_state( node, strategy_state )